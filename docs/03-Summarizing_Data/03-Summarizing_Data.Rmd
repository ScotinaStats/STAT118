...
## Examining .display[numerical] data
    
--

## Measures of .display[center] and .display[spread]
...
## Examining .display[categorical] data
...
gf_point(price ~ bedrooms, data = airbnb_boston, alpha = 0.4) + 
  labs(x = "Number of bedrooms", y = "Price per night (in dollars)", 
       title = "Boston Airbnbs")
```
]
...
.pull-left[
```{r, echo = FALSE}
gf_histogram( ~ price, data = airbnb_boston) + 
  labs(x = "Price per night (in dollars)")
```
]
...
data(famuss)
gf_histogram( ~ height, type = "count", data = famuss) + 
...
gf_histogram( ~ price, data = airbnb_boston) + 
  labs(x = "Price per night (in dollars)") + 
  geom_vline(xintercept = 163.4621, color = "purple", lty = 2, lwd = 2)
```
]
...
- **minimum**
- **first quartile (Q1)** (25th *percentile*: 25% of observations fall below Q1)
...
.pull-right[
*Width of box* represents **interquartile range** (IQR)
- IQR $=$ Q3 $-$ Q1
...
.center[
```{r, echo = FALSE, out.width = "45%"}
gf_boxplot( ~ price, data = airbnb_boston) + 
  labs(y = "Price per night (in dollars)")
```
]
...
gf_boxplot(price ~ room_type, data = airbnb_boston) +
  labs(y = "Price per night (in dollars)", 
...
.pull-left[
```{r, echo = FALSE}
gf_histogram( ~ house_income, type = "count", 
...
.pull-right[
```{r, echo = FALSE}
gf_boxplot( ~ house_income, xlab = "Annual Household Income")
```
]
